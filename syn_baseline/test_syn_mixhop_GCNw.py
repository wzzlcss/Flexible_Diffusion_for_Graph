...
parser.add_argument("--h", type=float, default=0.1, help="number of nodes per class in the synthetic dataset",)
# model training parameters
parser.add_argument('--cuda', type=int, default=0, help='Avaiable GPU ID')
parser.add_argument('--method', type=str, default='GCNw', help='which model to')
...
def double_aug_rw(adj, gamma):
    # (gamma * D + (1-gamma) * I)^-1 (gamma * A + (1-gamma) * I)
    N = adj.shape[0]
    adj.setdiag(0) # remove any self-loop
    rowsum = np.array(adj.sum(1)) + 1e-20
    aug_rowsum = gamma * rowsum + (1-gamma) * np.ones((N, 1))
    inv_aug_rowsum = np.power(aug_rowsum, -1.0).flatten()
    inv_aug_rowsum[np.isinf(inv_aug_rowsum)] = 0.
    aug_d_mat_inv = sp.diags(inv_aug_rowsum, 0)
    #d_mat = sp.diags(rowsum.flatten(), 0)
    aug_W = aug_d_mat_inv @ (gamma * adj + (1-gamma) * sp.eye(N))
    return aug_W
...
for gamma_10 in range(1, 11):
    gamma = gamma_10/10
    aug_W = double_aug_rw(adj, gamma)
    for split_idx in range(args.run):
        # prepare data
        #split = torch.load(os.path.join(BASE_DIR, f"500_split_{split_idx}.pt"))
        ...
        data_loader = DataLoader(aug_W, train_nodes, valid_nodes, test_nodes, device)
        #select model
    ...
    filename = f'./syn_results/syn_results_GCNw_l2.csv'
