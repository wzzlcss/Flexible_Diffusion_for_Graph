...
def get_args():
    ...
    parser.add_argument('--save_dir', type=str, default='new_heter_results_add', help='Base directory for saving information.')
    parser.add_argument('--dataset', type=str, default='chameleon-filtered-directed',
...
# create edges to the gradient step node
vf = direction_info.numpy()
gs_node = list(vf).index(vf.max())
# vf_mtx = vf - vf[:, np.newaxis]
# gs_neighbor = np.argmax(vf_mtx, axis=1)
src_nodes = [i for i in range(nnodes) if (i != gs_node and adj[gs_node, i] == 0)]
tgt_nodes = [gs_node] * len(src_nodes)
graph.add_edges(src_nodes, tgt_nodes)
dataset.graph = dgl.to_bidirected(graph.to('cpu')).to(args.device)
...
for layers in range(1, 6):
    ...
    filename = f'new_heter_results_add/{args.dataset}.csv'
